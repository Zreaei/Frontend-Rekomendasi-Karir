...
const UniversitySidebar = ({ collapsed, onToggle }: UniversitySidebarProps) => {
    ...
    >
      ...
      <div className={`grid grid-cols-[44px_minmax(0,1fr)] items-center gap-3 pr-8 transition-all duration-300 ease-in-out ${collapsed ? 'mx-0' : ''}`}>
        ...
        <div className={labelClass}>
          ...
          <p className="mt-0.5 text-[13px] text-white/70">Universitas</p>
        </div>
      </div>

      <nav className="grid gap-2 transition-all duration-300 ease-in-out flex-1 content-start" aria-label="University navigation">
        <NavLink to="/university" end className={({ isActive }) => `${navBase} ${navState} ${isActive ? navActive : ''}`}>
        ...
        <NavLink to="/university/manajemen-mahasiswa" className={({ isActive }) => `${navBase} ${navState} ${isActive ? navActive : ''}`}>
          <span className={iconBase} aria-hidden="true">
        ...
        <NavLink to="/university/manajemen-clo" className={({ isActive }) => `${navBase} ${navState} ${isActive ? navActive : ''}`}>
          <span className={iconBase} aria-hidden="true">
        ...
        <NavLink to="/university/manajemen-nilai" className={({ isActive }) => `${navBase} ${navState} ${isActive ? navActive : ''}`}>
          <span className={iconBase} aria-hidden="true">
        ...
        <NavLink to="/university/verifikasi-sertifikat" className={({ isActive }) => `${navBase} ${navState} ${isActive ? navActive : ''}`}>
          <span className={iconBase} aria-hidden="true">
